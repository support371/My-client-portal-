...
/**
 * ⚡ Bolt Optimization: Extract and memoize the Portfolio Summary section.
 * This ensures the complex UI within the summary doesn't re-render on parent
 * state changes (e.g., auth session updates).
 */
...
/**
 * ⚡ Bolt Optimization: Extract and memoize the Virtual Trading section.
 * Prevents the trading UI from re-rendering unnecessarily during parent state updates.
 */
...
/**
 * ⚡ Bolt Optimization: Extract and memoize the Transaction History section.
 * Since the `transactions` array is a module-level constant from `@/lib/data.ts`,
 * its reference remains stable, ensuring this component effectively skips re-renders.
 */
    ...
    <AuthGuard requiredRole="client">
      ...
      <main className="mx-auto max-w-5xl px-4 py-6 md:py-10">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          {/* ⚡ Bolt Optimization: Render extracted memoized sections. */}
          <PortfolioSummary />
          <VirtualTrading />
        </div>

        {/* ⚡ Bolt Optimization: Render extracted memoized transaction history. */}
        <TransactionHistory />
      </main>
    </AuthGuard>
  )
