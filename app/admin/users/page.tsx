...
// This prevents memoized components like PortalHeader from re-rendering due to prop identity changes.
...
const SHIELD_CHECK_ICON = <ShieldCheck className="h-4 w-4 text-primary" />
 ...
 * This ensures that only the relevant rows are re-rendered when the user list updates
 * or when unrelated parent state changes.
 */
...
export default function AdminUsersPage() {
  ...
  /**
   * ⚡ Bolt Optimization: Memoize the filtered user list.
   * Pre-normalizing the search query outside the filter loop improves performance from O(N*M) to O(N+M)
   * in terms of string normalization effort.
   */
