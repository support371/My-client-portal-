...
// This prevents memoized components from re-rendering due to new JSX element references.
 ...
 *
 * Prevents unnecessary re-renders of all rows when the search input or other
 * parent state changes. This is critical for maintaining performance as the
 * user list grows.
 */
...
export default function AdminUsersPage() {
  ...
  /**
   * ⚡ Bolt Optimization: Memoized Filtering with Normalized Query
   *
   * 1. Pre-normalizes the search query outside the filter loop to avoid O(N) calls to .toLowerCase().
   * 2. Uses useMemo to ensure the filter only runs when 'users' or 'search' changes.
   *
   * Impact: Reduces complexity from O(N * K) to O(N) where K is number of normalization ops.
   */
