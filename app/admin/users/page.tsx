...
// ⚡ Bolt Optimization: Hoist static JSX icons to module-level constants.
// This prevents redundant element creation and ensures stable references for memoized components.
const USERS_HEADER_ICON = <Users className="h-5 w-5 text-primary" />
...
/**
 * ⚡ Bolt Optimization: Memoized user row component.
 * Isolates re-renders to only the row that changed, and skips reconciliation
 * entirely when parent state (like search) updates but the user data remains stable.
 */
...
export default function AdminUsersPage() {
  ...
  // ⚡ Bolt Optimization: Memoize the filtered list to avoid O(n) re-calculations on every render.
  // We also normalize the search query once outside the filter loop for better performance.
