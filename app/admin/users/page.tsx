 ...
 * Prevents re-rendering all rows when the search input changes.
 * Only the list reconciliation will happen, and memoized components
 * will skip re-rendering if their props (user) haven't changed.
 */
...
export default function AdminUsersPage() {
  ...
  // This avoids O(N) string operations on every render if 'users' or 'search' hasn't changed.
                  ...
                  {filtered.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="py-8 text-center text-sm text-muted">
                        {users.length === 0 && !loadError ? "Loading…" : "No users match your search."}
                      </td>
                    </tr>
                  ) : filtered.map((user) => (
