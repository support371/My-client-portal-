 ...
 * Prevents unnecessary re-renders when unrelated page state (like search) changes.
 */
    ...
    <tr className="border-b border-border/50 hover:bg-surface/50">
      ...
      <td className="py-3 pr-4">
        <StatusBadge
          ...
          variant={user.active ? "success" : "critical"}
        />
      </td>
...
export default function AdminUsersPage() {
  ...
  // ⚡ Bolt Optimization: Memoize filtered results to prevent redundant O(N) calculations.
  // Performs normalization (toLowerCase) outside the filter loop for better performance.
  ...
  return (
    <AuthGuard requiredRole="admin">
      <PortalHeader title="User Management" icon={USERS_ICON} />
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    ...
                    filtered.map((user) => <UserRowComponent key={user.id} user={user} />)
                  )}
                </tbody>
              </table>
            </div>
