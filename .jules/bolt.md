...
## 2026-03-08 - [CI Workflow Consolidation]
**Learning:** Repositories can accumulate redundant or irrelevant CI workflows (e.g., Deno, Webpack, Azure boilerplate) that exhaust GitHub Actions resources and cause account-level billing locks. Consolidation into a single, project-specific workflow (e.g., Next.js + pnpm) improves maintenance and reliability.
**Action:** Identify and remove non-essential workflow files. Consolidate CI checks into a single optimized pipeline that matches the project's runtime and package manager.
